			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="regSheet.class_instructor.label" default="Classinstructor" /></th>
					
						<g:sortableColumn property="name" title="${message(code: 'regSheet.name.label', default: 'Name')}" />
				...
				<g:each in="${regSheetInstanceList}" status="i" var="regSheetInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${regSheetInstance.id}">${fieldValue(bean: regSheetInstance, field: "class_instructor")}</g:link></td>
					
						<td>${fieldValue(bean: regSheetInstance, field: "name")}</td>
